...
if [ "$TRAVIS_OS_NAME" = "windows" ]; then    
    ...
    powershell -File '.\publish-dapp.ps1' -wkdir "${BUILD_WIN_DIR}" -staticArtefactsDir "${ARTEFACTS_WIN_LOCATION}" -product VPN -installer -version "${VERSION_TO_SET_IN_BUILDER}" -forceUpdate "${DAPP_INSTALLER_FORCE_UPDATE}" -dappctrlConf "${DAPPCTRL_CONFIG}" -guiEthNetwork "${DAPP_GUI_NETWORK}" -dappguibranch "${GIT_BRANCH}" -dappctrlbranch "${GIT_BRANCH}" -dappinstbranch "${GIT_BRANCH}" -dappopenvpnbranch "${GIT_BRANCH}" -defaultBranch "${GIT_BRANCH_DEFAULT}"-Verbose
fi